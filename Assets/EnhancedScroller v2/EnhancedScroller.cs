...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public partial class EnhancedScroller : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
        ...
        public EnhancedScrollerCellView[] Cells;
        ...
        public bool ignoreChildForceExpandHeight;
        
        public bool ignoreChildForceExpandWidth;
        ...
        /// <returns></returns>
        public EnhancedScrollerCellView GetCellView(EnhancedScrollerCellView cellPrefab, int idx)
        {
        ...
        public void ReloadData(float scrollPositionFactor = 0)
        {
            if (!_initialized)
            {
                Debug.LogError("Enhanced还没有Awake初始化");
                return;
            }
            _reloadData = false;
        ...
        public void ReloadDataFromDataIndex(int dataIdx)
        {
            ...
            var cellPosition = GetScrollPositionForDataIndex(dataIdx, EnhancedScroller.CellViewPositionEnum.Before);

            // get the offset of the cell from the top of the scroll rect
            var tweenCellOffset = cellPosition - ScrollPosition;
            ...
            cellPosition = GetScrollPositionForDataIndex(dataIdx, EnhancedScroller.CellViewPositionEnum.Before);

            // set the scroller's position to focus on the cell, using the offset calculated above
            SetScrollPositionImmediately(cellPosition - tweenCellOffset);

            // turn loop jumping back on
            IgnoreLoopJump(false);
            ...
            _scrollPosition = GetScrollPositionForDataIndex(dataIdx, CellViewPositionEnum.Before);
            var scrollPositionFactor = _scrollPosition / ScrollSize;
            if (scrollDirection == ScrollDirectionEnum.Vertical)
            {
                // set the vertical position
                _scrollRect.verticalNormalizedPosition = 1f - scrollPositionFactor;
        ...
        void Awake()
        {
            ...
            _layoutGroup.childForceExpandHeight = !ignoreChildForceExpandHeight;
            _layoutGroup.childForceExpandWidth = !ignoreChildForceExpandWidth;
